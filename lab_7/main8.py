...
class WBCDDataset(Dataset):
    def __init__(self, fname, transform=None, target_transform=None):
        n_training = 40
        n_window = 30
        c = pd.read_pickle("press1.pkl")
        signal = c["pressure"].to_numpy()
        signal = (signal - np.mean(signal)) / np.std(signal)
        X, y, X_index, y_index = get_windows(signal, n=n_window)
        print(X.shape)
        self.X, self.y = X[:n_training], y[:n_training]

    def __len__(self):
        return len(self.X)

    def __getitem__(self, idx):
        data = np.array([*self.X[idx]], np.float32)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        print(output, target)
        loss = torch.nn.L1Loss().forward(output, target)
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            test_loss = torch.nn.L1Loss().forward(output, target).item()
...
def main():
    ...
    transform=transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((1000.0,), (1000.0,))
        ])
    # dataset1 = datasets.MNIST('../data', train=True, download=True,
    #                    transform=transform)
    # dataset2 = datasets.MNIST('../data', train=False,
    #                    transform=transform)
    dataset1 = WBCDDataset('data/wbcd_train.npz', transform=transform)
    dataset2 = WBCDDataset('data/wbcd_test.npz', transform=transform)
